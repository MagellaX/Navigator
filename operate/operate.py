...
def main(model, terminal_prompt, voice_mode=False, verbose_mode=False):
    """
    Main function for the Navigator.
    ...
    if terminal_prompt:  # Skip objective prompt if it was given as an argument
        ...
        print(
            f"{ANSI_GREEN}[Navigator]{ANSI_RESET} Listening for your command... (speak now)"
        )
        try:
        ...
        print(
            f"[{ANSI_GREEN}Navigator {ANSI_RESET}|{ANSI_BRIGHT_MAGENTA} {model}{ANSI_RESET}]\n{USER_QUESTION}"
        )
    ...
    while True:
        if config.verbose:
            print("[Navigator] loop_count", loop_count)
        try:
            ...
            print(
                f"{ANSI_GREEN}[Navigator]{ANSI_RED}[Error] -> {e} {ANSI_RESET}"
            )
            break
        except Exception as e:
            print(
...
def operate(operations, model):
    if config.verbose:
        print("[Navigator][operate]")
    for operation in operations:
        if config.verbose:
            print("[Navigator][operate] operation", operation)
        # wait one second
        time.sleep(1)
        ...
        operate_detail = ""
        if config.verbose:
            print("[Navigator][operate] operate_type", operate_type)
        ...
        if operate_type == "press" or operate_type == "hotkey":
            ...
            print(
                f"{ANSI_GREEN}[Navigator]{ANSI_RED}[Error] unknown operation response :({ANSI_RESET}"
            )
            print(
                f"{ANSI_GREEN}[Navigator]{ANSI_RED}[Error] AI response {ANSI_RESET}{operation}"
            )
            return True
